...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        #############################################################################
        pass
        # 课程视频中有提到，需要加max函数，同时求导也要考虑该函数
        hidden_layer = np.maximum(0, np.dot(X, W1) + b1)  # hidden:NxH
        scores = np.dot(hidden_layer, W2) + b2  # scores:NxC
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        s = np.exp(f) / np.exp(f).sum(axis=1).reshape(-1, 1)  # s.shape= NxC
